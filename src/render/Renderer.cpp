...
Renderer::Renderer(Window* window, PixelFormat pf)
  ...
  auto surfaceVertexShader =
      GlShader(shaders::kSurfaceVertexShader, GL_VERTEX_SHADER);
  auto surfaceFragmentShader =
      GlShader(shaders::kSurfaceFragmentShader, GL_FRAGMENT_SHADER);

  surfaceProgram_.init({surfaceVertexShader, surfaceFragmentShader});

  auto spriteVertexShader =
      GlShader(shaders::kSpriteVertexShader, GL_VERTEX_SHADER);
  auto spriteFragmentShader =
      GlShader(shaders::kSpriteFragmentShader, GL_FRAGMENT_SHADER);
  ...
  std::vector<float> surfaceVertices = {
      ...
      0,       0 + 200, 0.f, 1.f, 0 + 200, 0 + 200, 1.f, 1.f,
  };
...
void Renderer::setSize(uint32_t width, uint32_t height) {
  ...
  hpc_ =
      (static_cast<float>(width) / 2.f) / glm::tan(PI * 15.f * (1.f / 360.f));
  vpc_ =
      (static_cast<float>(height) / 2.f) / glm::tan(PI * 15.f * (1.f / 360.f));
  ...
  projectionMatrix_ =
      glm::perspective(70.f, width / static_cast<float>(height), 1.f, 2000.f);
  projectionMatrix2d_ = glm::ortho(0.f, static_cast<float>(width),
...
void Renderer::drawGround(const Camera& camera) {
  ...
  float time =
      std::chrono::duration_cast<std::chrono::duration<float>>(now - startTime_)
          .count();
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, groundTexture_.getWidth(),
               groundTexture_.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
               groundTexture_.getData());
...
void Renderer::drawSurfaces() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, charInfoTexture_.getWidth(),
               charInfoTexture_.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
               charInfoTexture_.getData());
